...
func (uq *UserQuery) QueryFaculty() *FacultyQuery {
	query := &FacultyQuery{config: uq.config}
...
func (uq *UserQuery) QueryBranch() *BranchQuery {
	query := &BranchQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryBuilding() *BuildingQuery {
	query := &BuildingQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithFaculty(opts ...func(*FacultyQuery)) *UserQuery {
	query := &FacultyQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withFaculty = query
	return uq
}

//  WithBranch tells the query-builder to eager-loads the nodes that are connected to
// the "branch" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithBranch(opts ...func(*BranchQuery)) *UserQuery {
	query := &BranchQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//  WithBuilding tells the query-builder to eager-loads the nodes that are connected to
// the "building" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithBuilding(opts ...func(*BuildingQuery)) *UserQuery {
	query := &BuildingQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withBuilding = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withFaculty; query != nil {
	...
	if query := uq.withBranch; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].branch_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
	...
	if query := uq.withBuilding; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].building_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "building_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Building = n
			}
		}
	}
